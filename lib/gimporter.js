...
gimporter.updateRecord = function (data,tableName) {
    ...
    data.indexado = "NO";
    data.is_new = "NO";
    connection.query('UPDATE ' + tableName +  ' SET ? WHERE cms_id = ?', [data,data.cms_id], function (err, rows, fields) {
      if (err) {
        throw err;
      }
      deferred.resolve();
...
gimporter.addRecord = function (data,tableName) {
    var deferred = Q.defer();
    connection.query('INSERT INTO ' + tableName + ' SET ?', data, function (err, rows, fields) {
      if (err) {
        throw err;
      }
      deferred.resolve();
...
gimporter.insertFile = function(data, tableName) {
  ...
  var q = connection.query('SELECT * FROM ?? WHERE ??=? LIMIT 1', [tableName, 'cms_id', data.cms_id], function (err, rows) {
    if (err) {
      throw err;
    } 
