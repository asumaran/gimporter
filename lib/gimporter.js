...
gimporter.insertFile = function(data) {
  ...
  connection.query('INSERT INTO legislations SET ?', data2, function (err, rows, fields) {
    if (err) {
      throw err;
    }
    deferred.resolve();
...
gimporter.getJurisprudenciaData = function() {
};
...
gimporter.processFile = function($, file) {
  ...
  this.insertFile(data).then(function(){
    deferred.resolve();
  });
